...
my$errsteptwo = system (`perl $steptwodir/steptwo.pl $steptwoinput $steptwooutput/auto_$samplename.sites_processed.csv`);
...
system(`mv tmp.bam tmp_$samplename.bam`);
system(`mv tmp.sam tmp_$samplename.sam`);
